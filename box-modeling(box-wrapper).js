(function ($) {
    $.fn.boxModeling = function (options) {
        ...
        const settings = $.extend({
            boxWrapperSelector: 'box-wrapper',
            boxSelector: 'box',
        ...
        return $(this).each(function () {
            ...
            const box = boxWrapper.querySelector('.' + settings.boxSelector);

            // $(boxWrapper).css('transform', $(boxWrapper).css('transform') + 'translate(-50%, -50%)');
            ...
            function repositionElement(x, y) {
                $(boxWrapper).css('left', x + 'px');
                $(boxWrapper).css('top', y + 'px');
            }
            ...
            function rotateBox(deg) {
                $(boxWrapper).css('transform', 'rotate(' + deg + 'deg)');
            }
            ...
            function resizeHandler(event, left, top, xResize, yResize) {
                ...
                initRotate = getRotation(boxWrapper);
            ...
            function editingStyle(event) {
                ...
                $(boxWrapper).css('z-index', getComputedStyle(document.body).getPropertyValue('--zi-' + $(boxWrapper).data('id')) );

                if ($(event.target).hasClass(settings.boxSelector)) {
                    $(event.target).find('.' + settings.handlerClass).show();
                    $(event.target).parents('.' + settings.boxWrapperSelector).css('z-index', '1000');
                ...
                if (!settings.resize) {
                    $($(event.target).parents().find('.resize-handler.resize').hide());
                }
                if (!settings.rotate) {
                    $($(event.target).parents().find('.resize-handler.rotate').hide());
                }
